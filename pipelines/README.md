...
| `02-silver/build_raster_mosaics.sh`                          | 02-silver | shell  | Per-date MosaicJSON stitching same-day Sentinel COG granules (the 4 scene collections; flood excluded by default) → R2. Reads hrefs from the catalog, so run it **after** the gold step | `bash <path>`                 |
